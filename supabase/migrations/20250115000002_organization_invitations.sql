...
CREATE OR REPLACE FUNCTION public.get_user_with_relations(user_id UUID DEFAULT NULL)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
...
COMMENT ON FUNCTION public.get_user_with_relations(UUID) IS 'Returns enriched user data with first_name, last_name, role, and related entities (cyclist or organizer with organization). If no user_id provided, returns data for the authenticated user. Organization now returns state instead of is_active.';
