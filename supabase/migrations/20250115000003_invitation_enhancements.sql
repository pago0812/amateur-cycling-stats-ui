...
COMMENT ON FUNCTION public.create_user_with_organizer_owner IS 'Creates or updates a user with organizer_owner role and links them to an organization';

-- Create RPC function for creating organizer staff users
CREATE OR REPLACE FUNCTION public.create_user_with_organizer_staff(
  p_auth_user_id UUID,
  p_first_name TEXT,
  p_last_name TEXT,
  p_organization_id UUID
...
BEGIN
  ...
  IF v_user_id IS NOT NULL THEN
    ...
    DELETE FROM public.cyclists WHERE user_id = v_user_id;
  ELSE
    -- Create new user with organizer_staff role
    INSERT INTO public.users (auth_user_id, first_name, last_name, role_id, created_at, updated_at)
    VALUES (p_auth_user_id, p_first_name, p_last_name, v_role_id, NOW(), NOW())
    RETURNING id INTO v_user_id;
  END IF;
...
COMMENT ON FUNCTION public.create_user_with_organizer_staff IS 'Creates or updates a user with organizer_staff role and links them to an organization';

-- Create RPC function for creating cyclist users
CREATE OR REPLACE FUNCTION public.create_user_with_cyclist(
  p_auth_user_id UUID,
  p_first_name TEXT,
  p_last_name TEXT
)
...
BEGIN
  ...
  IF v_user_id IS NOT NULL THEN
    -- User exists (created by trigger), just update names if needed
    UPDATE public.users
    SET
      first_name = p_first_name,
      last_name = p_last_name,
    ...
    WHERE id = v_user_id;
  ELSE
    -- Create new user with cyclist role
    INSERT INTO public.users (auth_user_id, first_name, last_name, role_id, created_at, updated_at)
    VALUES (p_auth_user_id, p_first_name, p_last_name, v_role_id, NOW(), NOW())
    RETURNING id INTO v_user_id;
  END IF;

  -- Ensure cyclist profile exists
  IF NOT EXISTS (SELECT 1 FROM public.cyclists WHERE user_id = v_user_id) THEN
    -- Get default gender
    SELECT id INTO v_default_gender_id FROM public.cyclist_genders LIMIT 1;

    -- Create cyclist profile
    INSERT INTO public.cyclists (user_id, gender_id)
    VALUES (v_user_id, v_default_gender_id);
  END IF;
...
COMMENT ON FUNCTION public.create_user_with_cyclist IS 'Creates or updates a user with cyclist role and ensures cyclist profile exists';
