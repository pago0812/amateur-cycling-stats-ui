	...
	<div class="mx-auto max-w-6xl p-8">
		<div class="mb-12">
			...
			>
				...
				<code class="rounded bg-amber-100 px-1.5 py-0.5 dark:bg-amber-900/40">goto()</code>
				from
				<code class="rounded bg-amber-100 px-1.5 py-0.5 dark:bg-amber-900/40">$app/navigation</code>
				which cannot be mocked in Storybook. The examples below show visual appearance only.
			</div>
		</div>
			...
			<div class="space-y-6">
				<div class="rounded-lg border border-border bg-card p-6">
					...
					<p class="mb-4 text-sm text-muted-foreground">
						When filters match an existing race, navigates with <code
							class="rounded bg-muted px-1.5 py-0.5">?raceId=UUID</code
						>
					</p>
					<RaceFilterSelect event={mockEventWithRaces} currentRaceId="race-1" />
					<p class="mt-4 text-xs text-muted-foreground">
						Example URL: <code class="rounded bg-muted px-1.5 py-0.5"
							>/results/202?raceId=race-1</code
						>
					</p>
				</div>
    ...
    Navigate (SSR reload)</code
		...
		<section class="mb-12">
			...
			<ul class="list-disc space-y-2 pl-6 text-muted-foreground">
				...
				<li>
					SSR-friendly navigation with <code class="rounded bg-muted px-1.5 py-0.5"
						>invalidateAll: true</code
					>
				</li>
...
&lt;RaceFilterSelect &#123;event&#125; &#123;filterState&#125; /&gt;</code
					></pre>
			</div>
		</section>
	</div>
