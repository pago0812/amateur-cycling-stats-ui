<script lang="ts">
	...
	$effect(() => {
		...
		// Start new timer when alert opens
		if ($alertStore.open) {
...
{#if $alertStore.open}
	<div class="fixed top-4 left-1/2 z-50 w-full max-w-md -translate-x-1/2 transform px-4">
		<div
			class="flex items-center justify-between rounded-lg {bgColor} px-6 py-4 text-white shadow-lg"
			role="alert"
		>
			<div class="flex items-center gap-3">
				...
				{/if}
				<span class="font-medium">{$alertStore.text}</span>
			</div>
			<button
				onclick={handleClose}
