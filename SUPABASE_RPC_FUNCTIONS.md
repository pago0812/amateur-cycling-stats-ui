...
2. [get_cyclist_by_user_short_id](#2-get_cyclist_by_user_short_id)
3. [get_race_results_by_user_short_id](#3-get_race_results_by_user_short_id)
...
```typescript
{
  user_short_id?: string  // Optional - If provided, lookup by short_id; otherwise use current session
}
```
...
## 2. get_cyclist_by_user_short_id
...
```typescript
{
	p_user_short_id: string; // Required - User's public short_id
}
```
...
┌─────────────────────────────────────┐
│  supabase                           │
│  .rpc('get_cyclist_by_user_short_id'│
│    { p_user_short_id: id })         │
└──────────┬──────────────────────────┘
           │
           ▼
...
## 3. get_race_results_by_user_short_id
...
{
	"success": true,
	"organization_short_id": "org123abc45",
	"user_id": "abc123def4",
...
const [cyclist, raceResults] = await Promise.all([
	getCyclistById(id), // get_cyclist_by_user_short_id
	getRaceResultsByUserId(id) // get_race_results_by_user_short_id
]);
...
WHERE rr.user_id = (SELECT id FROM users WHERE short_id = p_user_short_id)
```
...
| ----------------------------------------------- | -------------------------------------------------------------------- |
| `20250115000001_core_foundation.sql`            | `get_race_results_by_user_short_id`, `get_cyclist_by_user_short_id`  |
...
// get_auth_user
type GetUserParams = {
	user_short_id?: string; // Optional
};

// get_cyclist_by_user_short_id / get_race_results_by_user_short_id
type GetByShortIdParams = {
	p_user_short_id: string; // Required
};
...
// get_cyclist_by_user_short_id (returns Cyclist after adapter)
type Cyclist = {
	id: string;
	authUserId: string;
	firstName: string;
...
// get_race_results_by_user_short_id (returns RaceResultWithRelations[] after adapter)
...
2. ✅ **User Data Fetching** - `get_cyclist_by_user_short_id`
3. ✅ **Race Results** - `get_race_results_by_user_short_id`
